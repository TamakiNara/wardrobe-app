...
- `standard-user@example.com` は手書き 27 件の Item（うち `disposed` 1 件、`in_cleaning` 1 件、ボトムス丈 5 種とレッグウェア被覆 8 種、`onepiece` / `allinone` 確認用 item を含む）と 12 件の Outfit（うち `invalid` 2 件）、8 件の wear logs を持ち、`large-user@example.com` は Factory 併用の 36 件の Item と 12 件の Outfit、14 件以上の wear logs を持つ
- `standard-user@example.com` の Outfit には `ワンピース重ねコーデ`、`ワンピースパンツ重ねコーデ`、`トップス下ワンピースパンツコーデ`、`トップス上ワンピースパンツコーデ`、`オールインワン羽織りコーデ`、`旧オールインワン羽織りコーデ` を追加し、tops と onepiece / allinone の重なり順、ワンピースの下にパンツを重ねる許容ケース、手放し済み item を含む invalid outfit を配色差つきで確認できるようにした
