...
- `onepiece_allinone` は item 単体では共通四角 SVG を維持しつつ、outfit サムネイルでは `bottoms` がない場合に限って全高レイヤー候補として扱う
- tops と `onepiece_allinone` の上下関係は `sort_order` の大きい item を上側レイヤーとして解決し、`legwear` は下側の lower-body preview 専用責務を維持する
- `onepiece + bottoms` は許容する前提とし、後続では `onepiece` を主レイヤー、`bottoms` を裾見せの補助レイヤーとして簡略化する方針にした
- `allinone + bottoms` は `onepiece` と同列に確定せず、表示ルールは要再判断のまま残す
- current では `bottoms` 併用時に `onepiece_allinone` レイヤーへ寄せず通常レイアウトを維持し、wear log への同ルール適用は後続とする
