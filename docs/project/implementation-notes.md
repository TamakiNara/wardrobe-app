...
8-1. 対応済み: 購入検討一覧の filter UI とカード比較性の整理
   - 現状: 購入検討一覧では keyword / status / priority / category / sort / brand の filter UI とページング UI を一覧上で自然に使える形へ整理済み
...
8-2. 将来タスク: 購入検討一覧の色違い対応整理
   - 種別: UI/UX改善
   - 優先度: 中
   - 背景: カラーチップを維持したことで、同一商品の色違い候補が増えた時の扱いを整理する必要がある
   - やりたいこと: 一覧の判別軸としてカラーチップは維持しつつ、色違いを別候補のまま並べるか、同一商品としてまとめるか、価格差や状態差をどう扱うかを仕様として整理する
   - 保留事項:
...
### 要再判断: wear logs 周辺の DB 見直し

- snapshot なし継続を前提に、`wear_log_items.source_item_id` から current item を参照できる状態を維持する
- そのため、`items` は原則物理削除しない方針を第一候補にし、UI 上の `削除` を `disposed` / `inactive` / `deleted_at` のどれで扱うかを後続で固定する
- `disposed item` は新規候補から除外しつつ、過去の wear log / outfit 参照では確認できる前提を維持する
- `wear_log_items` は wear log 内 item 構成の正本として扱い、`source_outfit_id` は item 構成の正本ではなく由来参照であることを維持する
- DB 制約は `(wear_log_id, source_item_id)` unique、`(wear_log_id, sort_order)` unique、`sort_order` の null 禁止、`item_source_type = outfit / manual` の固定、FK と delete 方針を後続で再確認する
- `wear_logs.status` は `planned / worn` のまま責務を増やしすぎず、`cancelled` 追加や `invalid` / `過去未完了` / `候補外 item 含む` などは別フラグや補助表示で扱う方針を維持する
- `wear_logs` 削除時に `wear_log_items` が孤児化しないことを、FK cascade delete と migration / 実装の両面で再確認する
