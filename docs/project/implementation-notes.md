...
2-1. 購入検討から item 作成へ進んだ時の画像表示整理
   - 背景: candidate 画像と item 側画像の役割は分かれているが、UI 上では少し混線して見える
   - やりたいこと: item 作成画面で引き継ぎ画像の見せ方を整理し、同じ画像が 2 箇所に見える状態を見直す
   - 保留事項: 補助表示として残すのか、item 側画像欄へ統合するのか
2-2. 購入検討の複製後導線見直し
   - 背景: 複製後に詳細画面へ遷移し、コピーであることも分かりにくい
   - やりたいこと: 複製後の遷移先を見直し、新しい購入検討として再利用しやすい導線にする。コピー識別方法（名前、ラベル、メッセージ等）も検討する
   - 保留事項: 編集画面遷移にするか、詳細画面のままメッセージで補うか、`(コピー)` 命名を採用するか
3. 比較結果の扱いを整理する
