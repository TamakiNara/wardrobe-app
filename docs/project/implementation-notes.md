...
6. item SVG の簡略化方針を docs と画面で揃える
   - tops / dress の item SVG は shape 個別表現を増やさず、カテゴリ単位の記号化を維持する
...
- wear log での脚見え合成表現、one-piece / dress 対応、tights / stockings のサブカラー固定は planned のままとし、正本は `docs/specs/items/thumbnail-skin-exposure.md` を参照する
- one-piece / dress は item 単体では共通四角 SVG を維持しつつ、outfit サムネイルでは `bottoms` がない場合に限って全高レイヤー候補として扱う
- tops と one-piece / dress の上下関係は `sort_order` の大きい item を上側レイヤーとして解決し、`legwear` は下側の lower-body preview 専用責務を維持する
- `bottoms` 併用時は dress 専用レイヤーへ寄せず current の通常レイアウトを維持し、wear log への同ルール適用は後続とする
- outfit サムネイルの色帯レイアウトでは `legwear` を `others` から除外し、lower-body preview 側だけで扱うように寄せた。`others` は non-lower-body の残余カテゴリを指し、one-piece / dress 後続整理でも `legwear = lower-body 専用` を前提にする
