...
- one-piece / dress は current では item 単体のみ共通四角 SVG に寄せ、outfit / wear log の合成ではまだ `others` 扱いのままにしている。後続では tops との上下関係を `sort_order` 正本で決め、legwear は下側レイヤーとして流用する方針で整理する
