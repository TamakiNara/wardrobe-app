...
- wear log thumbnail の current code は `WearLogColorThumbnail` / `WearLogModalColorThumbnail` から representative selection / mode resolution / ViewModel build を呼ぶ構成に整理済みだが、mode 自体はまだ `standard` 固定である
...
- wear log の `onepiece_allinone` 専用 mode は未導入で、current は `standard` mode のまま lower-body preview だけを改善している
