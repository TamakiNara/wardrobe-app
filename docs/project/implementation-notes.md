...
- `user_preferences` に `currentSeason` / `defaultWearLogStatus` / `calendarWeekStart` を保存できる
- settings 画面でカテゴリ表示設定の取得・保存ができる
- settings 画面で `currentSeason` / `defaultWearLogStatus` / `calendarWeekStart` の取得・保存ができる
...
- wear logs の日詳細モーダル / 個別詳細には、まだ配色サムネイルを出していない
- wear logs の日詳細モーダルへ配色サムネイルを広げる場合の第一候補は、`wear_log_items` 正本を維持した簡略版 component を一覧用 helper ベースで追加する形
