...
- Phase 2-3 で outfit thumbnail の lower-body preview を実装し、表示対象 item から `sort_order` 昇順で representative bottoms / legwear を選定して item 側 lower-body 描画ルールを再利用する
- outfit thumbnail の current は representative bottoms がある場合のみ lower-body preview を表示し、representative legwear は有効 spec がある最初の候補だけを合成に使う
