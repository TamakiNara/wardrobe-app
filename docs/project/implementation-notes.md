...
- wear log thumbnail の current code は `WearLogColorThumbnail` / `WearLogModalColorThumbnail` + `buildWearLogThumbnailLayout` の単純構成で、outfit thumbnail のような representative selection / mode resolution / ViewModel build の責務分離はまだ持っていない
- wear log thumbnail は current では `buildColorThumbnailLayout` をそのまま使うため、`onepiece_allinone` も `legwear` も専用扱いせず、`tops` / `bottoms` / `others` の既存色帯レイアウトへ流している
- current の wear log API は、後続の representative selection / mode resolution に備えて `thumbnail_items` へ `shape` / `spec` / `sort_order` まで返すが、描画側はまだ category + colors 中心の既存レイアウトを維持する
- wear log thumbnail 実装着手前タスクは、1) API で `thumbnail_items` に `shape` / `spec` / `sort_order` を追加し、`skinTonePreset` は lower-body preview 導入時に必要か再判定、2) web 型を追随、3) `wear_log_items` 正本の representative selection / mode resolution / lower-body preview helper を追加、4) wear log thumbnail component に ViewModel と描画分岐を導入、5) API / helper / component / integration test を順に増やす、の依存順で進める
