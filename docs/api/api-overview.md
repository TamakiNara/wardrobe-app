...
- `user_preferences` は `currentSeason` / `defaultWearLogStatus` / `calendarWeekStart` の保存先で、1 user = 1 record を前提とする
...
- `GET /api/settings/preferences` は未設定時に `null` を返す
- `PUT /api/settings/preferences` は `currentSeason` / `defaultWearLogStatus` / `calendarWeekStart` を更新し、`message=updated` と更新後 preferences を返す
