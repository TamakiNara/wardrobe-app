...
| `updated`                  | `ItemResponseWithMessage`, `OutfitResponseWithMessage`, `WearLogMutationResponse`, `PurchaseCandidateResponseWithMessage`, `UserBrandResponseWithMessage`, `CategoryVisibilitySettingsUpdatedResponse`, `UserPreferencesUpdatedResponse` | `PUT /api/items/{id}`, `PUT /api/outfits/{id}`, `PUT /api/wear-logs/{id}`, `PUT /api/purchase-candidates/{id}`, `PATCH /api/settings/brands/{id}`, `PUT /api/settings/categories`, `PUT /api/settings/preferences`             | あり         | response に含まれる更新後 record / 設定値 / preferences |
...
| `updated`                  | `ItemResponseWithMessage`                                                                                                                                                                                                                | `POST /api/items/{id}/care-status`                                                                                                                                                                                             | あり         | response に含まれる更新後 item                          |
...
- OpenAPI には、現状の実装でアプリ固有の意味や読み分けがある error response を優先して明示する
...
- 現状の API では、競合系・業務ルール違反の失敗は原則として `409` ではなく `422` に寄せる
...
- 新規 API で 409 を増やす前に、まず `422 + errors` と `422 + message only` のどちらで 現状の方針に沿って表現できるかを確認する

### 401 / 404 Rules

- `401` は未ログイン・認証切れなど、認証状態を満たしていないことを表す
- 現状の API では、未認証時は原則として JSON の `401 Unauthenticated.` を返す
- `404` は純粋な未検出に加え、他人データ秘匿のためにも使う
- 現状の API では所有者チェック付きの `findOrFail` を多用しているため、他人データアクセスは `403` ではなく `404` になる
...
- 現在の OpenAPI では `400` / `429` / `500` を原則として明示していない
- これらは 現状の実装でアプリ独自の response shape や文言を定義していないため、framework default に委ねる
- `400`
  - malformed JSON や想定外 request 形式のように、アプリ固有の業務ルールではない失敗に留まる限り、個別 endpoint に明示しない
- `429`
  - 現状の API では endpoint 単位の明示的な rate limit 契約を docs 化していないため、原則省略する
...
- `user_preferences` は `currentSeason` / `defaultWearLogStatus` / `calendarWeekStart` / `skinTonePreset` の保存先で、1 user = 1 record を前提とする
...
- `GET /api/settings/preferences` は `currentSeason` / `defaultWearLogStatus` / `calendarWeekStart` の未設定時に `null` を返し、`skinTonePreset` は `neutral_medium` を返す
- `PUT /api/settings/preferences` は `currentSeason` / `defaultWearLogStatus` / `calendarWeekStart` / `skinTonePreset` を更新し、`message=updated` と更新後 preferences を返す
- `currentSeason` の保存値は `spring / summer / autumn / winter` で、item / コーディネート一覧の既存季節 filter 値は `春 / 夏 / 秋 / 冬 / オール` のまま維持する
- item 一覧 / コーディネート一覧では、URL に季節条件がないときだけ `currentSeason` を初期値として使う
- wear log 新規作成画面では `defaultWearLogStatus` を初期値に使い、edit では既存 record を優先する
- wear log カレンダーの週開始は `calendarWeekStart` で切り替え、未設定時は月曜始まりを既定とする
- `skinTonePreset` は item サムネイルの肌色表現にのみ使い、outfit / wear log 反映は後続対応とする
- settings 画面の週開始選択は `月曜始まり / 日曜始まり` の2択で見せる
...
- `spec` は nullable
- 現在の実装では `spec.tops` / `spec.bottoms.length_type` / `spec.legwear.coverage_type` を利用
- `spec.legwear.coverage_type` は `legwear` category の item で扱い、`shape` は粗い分類として併用する
- `spec.legwear.coverage_type` は `shape` ごとに許可値があり、不整合な組み合わせは 422 になる
- `spec.tops.*` / `spec.bottoms.length_type` / `spec.legwear.coverage_type` は create / update の両方で受け付ける
- `images` は `sort_order` と `is_primary` を含む配列として create / update で受け付け、item 編集画面の並び替え / 代表画像切り替えに利用する
- item picker や outfit detail の使用アイテム表示では、`images[].url` がある登録済み写真だけをサムネイル候補に使う。primary image を優先し、なければ `sort_order` が最小の画像を使う
- `care_status` は補助状態として `in_cleaning / null` を受け付ける
...
- item の delete は物理削除の例外導線で、通常の所有状態変更は `dispose` / `reactivate` を主導線とする
- `outfit_items` / `wear_log_items` から参照されている item は delete せず、`disposed` への状態変更を優先する
...
- `care-status` 更新は item の補助状態だけを変え、候補除外や invalid 化の主制御には使わない
- `in_cleaning` item は wear log で planned / worn ともに保存可能で、UI では警告だけを出す
...
- `invalid outfit` 一覧、`restore`、`duplicate` は 現状の実装として利用できる
...
- `outfit_items[].item` は `ItemRecord` として扱い、detail payload では登録済み item 画像の `url` も使用アイテム写真サムネイルに利用できる
...
- `items=[]` かつ `source_outfit_id` ありで保存した場合、現時点の実装では source outfit の構成を `wear_log_items` として実体化する
- `item_source_type` は `outfit` / `manual`
- `GET /api/wear-logs/calendar` は月カレンダー表示専用の軽量 API で、日付ごとの planned / worn 件数と dot 表示用の最小情報だけを返す
- `dots` は `wear log 1件 = dot 1個` で、`planned / worn` のみを表し、警告状態やクリーニング中のような補助状態は含めない
- `GET /api/wear-logs` の `thumbnail_items` は一覧用の補助配色サムネイル描画向け最小情報で、`source_outfit_id` ではなく `wear_log_items` を正本として返す
- `GET /api/wear-logs/by-date` は日付クリック後の日詳細モーダル専用 API で、当日の wear log を `display_order asc` で返す
- `GET /api/wear-logs/by-date` も、モーダル用の簡略版配色サムネイル描画に使う `thumbnail_items` を返し、正本は `wear_log_items` のままとする
- `GET /api/wear-logs/{id}` は個別詳細の正本で、`source_item_status` に加えて `source_item_care_status` も返し、warning 表示と status 変更判断に使う
- 空の日でも `GET /api/wear-logs/by-date` を呼び、空状態モーダルと `この日で新規作成` 導線を表示する
- delete は `MessageOnlyResponse` を返し、削除結果の正本は後続の一覧 / 詳細取得結果で確認する
...
- `item-draft` は candidate 側の `category_id` を source metadata として保持しつつ、現在の item API 互換の `category` / `shape` と配列項目を返す前提とする
- `item-draft` は candidate `memo` を item 作成画面の `memo` 初期値として含める一方、`wanted_reason` は item へ持ち込まない
- item 保存後は candidate `memo` と item `memo` を同期しない
- candidate から item へは登録済み全画像を引き継ぎ、`sort_order` と `is_primary` も維持する前提とする
- 現時点の実装では、item create に `purchase_candidate_id` を渡した場合、Laravel 側で item 作成、画像コピー、candidate の `purchased` 反映、`converted_item_id` / `converted_at` 更新をまとめて処理する
- `price` は通常価格、`sale_price` はセール価格 / 値引き価格として扱う
- `release_date` / `sale_price` / `sale_ends_at` / `discount_ends_at` は candidate 専用であり、list / detail では補助表示するが、`item-draft` や item create には含めない
- `sale_ends_at` は商品の販売・掲載終了日、`discount_ends_at` は `sale_price` が有効なセール終了日として扱う
- `POST /api/purchase-candidates/{id}/duplicate` は詳細画面から使う 現状の API で、`created` と新しい purchase candidate record を返す
- candidate 複製では colors / seasons / tpos / images を引き継ぎ、画像は新 candidate 用保存先へ物理コピーする
- `purchased` の candidate は item 化済み履歴として扱い、candidate 側更新を item へ逆流させない
- `purchased` の candidate 更新では `memo` / `wanted_reason` / `priority` / `release_date` / `sale_price` / `sale_ends_at` / `discount_ends_at` / `purchase_url` のみ許可し、それ以外の項目を送ると `422 ValidationErrorResponse` を返す
- `purchased` の candidate では `item-draft` を生成せず、再検討したい場合は duplicate を使う
- duplicate は `considering` / `on_hold` / `purchased` / `dropped` のどの status からでも実行できる
