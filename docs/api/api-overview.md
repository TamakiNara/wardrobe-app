...
- `user_preferences` は `currentSeason` / `defaultWearLogStatus` の保存先で、1 user = 1 record を前提とする
...
- `PUT /api/settings/preferences` は `currentSeason` / `defaultWearLogStatus` を更新し、`message=updated` と更新後 preferences を返す
