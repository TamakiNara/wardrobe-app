...
- `bags_tote` : トートバッグ
- `bags_shoulder` : ショルダーバッグ
- `bags_boston` : ボストンバッグ
- `bags_rucksack` : リュックサック・バックパック
- `bags_hand` : ハンドバッグ
- `bags_waist_pouch` : ウエストポーチ
- `bags_messenger` : メッセンジャーバッグ
- `bags_clutch` : クラッチバッグ
- `bags_sacoche` : サコッシュ
- `bags_pochette` : ポシェット
- `bags_drawstring` : ドローストリングバッグ
- `bags_basket_bag` : かごバッグ
- `bags_briefcase` : ブリーフケース
- `bags_marche_bag` : マルシェバッグ
- `bags_body` : ボディバッグ
- `bags_other` : その他バッグ
...
- `bags`
  ...
  - 一方で現在の item shape としては `bag / tote / shoulder / rucksack / clutch` 程度を持ち、用途差は shape で吸収する
- `kimono`
...
- current では `bags_tote / bags_shoulder / bags_boston / bags_rucksack / bags_hand / bags_body / bags_waist_pouch / bags_messenger / bags_clutch / bags_sacoche / bags_pochette / bags_drawstring / bags_basket_bag / bags_briefcase / bags_marche_bag / bags_other` を中分類として持つ
- item でも `subcategory = tote / shoulder / boston / rucksack / hand / body / waist_pouch / messenger / clutch / sacoche / pochette / drawstring / basket_bag / briefcase / marche_bag / other` を主導線にし、一覧・検索・category settings と同じ粒度で扱う
- `shape` は current では各種類に対応する1件の候補を自動補完する薄い補助値として扱い、必要になるまで厚くしない
...
- current では `inner` の item も `subcategory = roomwear / underwear / pajamas / other` を主導線として扱い、`shape` は同名1件の候補を自動補完する薄い補助値として扱う
...
分類プリセットは、登録直後のカテゴリ表示設定 `visible_category_ids` の初期値を決めるためのものとする。

補足:

- UI 表示は `メンズ / レディース / カスタム` とする
- 内部値は現行実装に合わせて `male / female / custom` を維持する
- 将来的に `men / women / custom` へ寄せるかは別途検討する
- プリセットは「日常的に使う可能性が高いカテゴリを初期 ON にする」ためのものであり、すべての性別表現や服装パターンを固定するものではない
- 初期 OFF のカテゴリも、設定画面から後で ON にできる
...
メンズプリセットでは、一般的な日常服として利用頻度が高いカテゴリを初期 ON にし、女性向け・低頻度・用途限定になりやすいカテゴリは初期 OFF にする。
...
- シューズ大分類自体は初期 ON とし、パンプスのみ初期 OFF とする
- レッグウェア大分類自体は初期 ON とし、ソックスなどは利用候補として残す
- ストッキング / タイツは、必要な場合に設定画面で ON にできる
- 水着 / 着物は、性別に関わらず日常利用頻度が低いため初期 OFF とする
...
レディースプリセットでは、一般的な日常服として利用頻度が高いカテゴリを初期 ON にする。

ただし、カテゴリ数増加により初期表示が重くなるため、すべて ON ではなく、低頻度・用途限定になりやすいカテゴリは初期 OFF にする。
...
- 水着 / 着物は、女性でも所持していない可能性が高く、日常のクローゼット管理では初期表示に含めない方が自然である
- 必要な場合は設定画面で ON にできる
- スカート / ワンピース・ドレス / オールインワン / パンプス / ストッキング / タイツは、レディースプリセットでは初期 ON の第一候補とする
...
カスタムプリセットでは、ユーザーが自分でカテゴリ表示を調整する。

初期状態は「すべて ON」ではなく、主要カテゴリ ON / 低頻度カテゴリ OFF を第一候補とする。
...
- カスタムは、ユーザーが自分で調整する前提のため、最初から全カテゴリ ON にするとカテゴリ数が多くなりすぎる
- 一方で、すべて OFF から始めると初回設定負荷が高いため、主要カテゴリは ON の状態から始める
- 水着 / 着物のような低頻度カテゴリは初期 OFF とし、必要な場合に ON にしてもらう
...
- `custom` を選んだ場合は、custom プリセットの初期 ON / OFF 状態でカテゴリ微調整画面へ進む
- カスタム調整画面では、プリセット選択画面へ戻る導線を表示する
...
プリセットは大分類単位だけでなく、必要に応じて中分類単位でも初期 ON / OFF を制御する。

理由:

- 大分類全体は必要でも、一部の中分類だけ不要なケースがあるため
- 例として、メンズプリセットではレッグウェア自体は必要だが、ストッキング / タイツは初期 OFF の方が自然である
- 同様に、シューズ自体は必要だが、パンプスはメンズプリセットでは初期 OFF の方が自然である

初期 OFF の代表例:

| プリセット | 大分類 | 中分類 | 方針 |
| ---------- | ------ | ------ | ---- |
| メンズ | シューズ | パンプス | 初期 OFF |
| メンズ | レッグウェア | ストッキング | 初期 OFF |
| メンズ | レッグウェア | タイツ | 初期 OFF |
| メンズ / レディース / カスタム | 水着 | すべて | 初期 OFF |
| メンズ / レディース / カスタム | 着物 | すべて | 初期 OFF |

補足:

- 現行カテゴリではレッグウェアが未導入または再編候補扱いの場合があるため、実装時は現行 category master の ID に合わせて適用する
- カテゴリ再編後は `legwear_stockings` / `legwear_tights` のような中分類 ID 単位で制御する
- 現行実装に存在しないカテゴリ ID は、docs 上では再編後の方針として扱い、即時実装対象にしない
