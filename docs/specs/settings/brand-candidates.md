...
- ただし現時点では過剰設計を避け、初期実装範囲では入力補助を主目的にする
...
- キーワードでブランド名 / 読み仮名の両方を絞り込める
- 有効候補を主表示とし、無効候補は必要時に確認できる形を優先する
- 無効候補は折りたたみ表示で整理し、既定では閉じてよい
...
- `DELETE /api/settings/brands/{id}`
- 候補削除前提の運用 seed や、より広いブランドマスタ風 seed の充実
