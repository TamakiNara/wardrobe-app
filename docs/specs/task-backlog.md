...
| UI-06 | custom dropdown / popover とボトムナビの重なり確認             | current では AppShell に bottom nav 分の `pb-[calc(5rem+env(safe-area-inset-bottom))]` があり、GlobalBottomNav は `z-50`。通常フロー内の confirmation UI は構造上 bottom nav に隠れにくい。ColorSelect と BrandFilter dropdown は `z-[60]`、wear-log calendar modal は `z-[70]` とし、明確に bottom nav 背面へ潜るリスクがあった BrandFilter は修正済み。remaining は SP 幅目視確認、viewport 下端での見切れ、placement / max-height / safe-area / Portal 要否、z-index ルール正式化。 | 手元タスク, ColorSelect dropdown と bottom nav 干渉修正の運用メモ, BrandFilter dropdown z-index 修正                                                                          | shared ui / navigation               | ui   | low    | 新しい dropdown / popover UI の追加時          | yes              |
| UI-07 | native confirm のアプリ内確認 UI への置き換え                  | `window.confirm()` を使っている重要操作を棚卸しし、削除・解除・復元・設定変更などはアプリ内確認 UI へ段階置き換えする。item delete / purchase candidate delete / outfit delete / wear log delete / weather record delete / weather location delete / shopping memo item remove / item dispose / settings categories は current で対応済み。runtime code の native confirm 残件はなく、alert / prompt 系は UI-10 / UI-11 と logout failure inline error 化で整理済み。                  | `items/delete-policy.md`, `purchase-candidates.md`, `outfits/create-edit.md`, `wears/wear-logs.md`, `items/status-management.md`, 手元確認メモ, `web/src` の confirm 使用箇所 | shared ui / destructive actions      | ui   | medium | current 対応済み                               | yes              |
| UI-08 | 購入検討削除前に買い物メモ所属による削除不可状態を事前表示する | current では purchase candidate detail / edit response に `is_used_in_shopping_memos` と `shopping_memo_count` を含め、編集画面の削除 UI で買い物メモ所属ありの場合に削除ボタンを disabled にし、削除前から理由を表示する。backend delete 時の `422` user-facing message は安全網として維持する。delete-check API は current では不要で、削除不可理由が増えた場合に再検討する。                                                                                                        | `purchase-candidates.md`, `shopping-memos.md`, `docs/api/openapi.yaml`                                                                                                        | purchase candidates / shopping memos | ui   | medium | current 対応済み / delete-check API は保留     | yes              |
...
| UI-10 | 401 alert + login redirect のアプリ内通知化                    | `window.alert()` を使っていた `401` / セッション切れ / ログイン再要求通知を、current では `/login?message=session_expired` への `router.replace` と login page 側のアプリ内通知へ置き換え済み。component 側は共通 helper を使う。logout failure alert も logout button 内の inline error 表示へ置き換え済み。`return_to` / `next` によるログイン後復帰は保留。                                                                                                                         | `web/src` の alert 使用箇所, 手元確認メモ                                                                                                                                     | auth / error notification            | ui   | medium | current 方針実装済み / ログイン後復帰は保留    | yes              |
