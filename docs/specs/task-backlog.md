...
| UI-07 | native confirm のアプリ内確認 UI への置き換え      | `window.confirm()` を使っている重要操作を棚卸しし、削除・解除・復元・設定変更などはアプリ内確認 UI へ段階置き換えする。item delete と purchase candidate delete は current で対応済みとし、outfit delete / wear log delete / shopping memo item remove / weather record delete / weather location delete / item dispose などを優先度順に整理する | `items/delete-policy.md`, `purchase-candidates.md`, 手元確認メモ, `web/src` の confirm 使用箇所                             | shared ui / destructive actions | ui   | medium | 共通 confirm UI パターン整理          | yes              |
...
| DES-07 | purchase candidate 削除と shopping memo 所属時の UX 整理      | `shopping_memo_items.purchase_candidate_id` が `restrictOnDelete` のため、買い物メモに含まれている購入検討を削除したときの current failure と、将来の `422 + 先に外してください` 方針を整理する | `purchase-candidates.md`, `shopping-memos.md`                                                                                                                                            | purchase candidates / shopping memos   | design | medium | current DB 制約維持 / backend 事前チェック方針判断 | needs design     |
...
- 購入検討削除と買い物メモ所属時の UX は、confirm 置き換えとは別論点として `DES-07` に分けた
