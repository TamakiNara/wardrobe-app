...
| UI-07 | native confirm のアプリ内確認 UI への置き換え      | `window.confirm()` を使っている重要操作を棚卸しし、削除・解除・復元・設定変更などはアプリ内確認 UI へ段階置き換えする。item delete は current で対応済みとし、outfit delete / wear log delete / purchase candidate delete / shopping memo item remove / weather record delete / weather location delete / item dispose などを優先度順に整理する | `items/delete-policy.md`, 手元確認メモ, `web/src` の confirm 使用箇所                                                       | shared ui / destructive actions | ui   | medium | 共通 confirm UI パターン整理          | yes              |
