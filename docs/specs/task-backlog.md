...
outfit list の使用アイテム filter / WearLogForm の item 選択 / purchase candidate 側の候補表示で、item 候補カードにブランド名を出すか検討する。同系統 item が増えると、カテゴリ / 形 / 色だけでは判別しづらくなるため、ブランド名も含めた表示情報を整理する

#### 検討観点

- outfit list の使用アイテム filter でブランド名を表示する。
- WearLogForm の item 選択でも同じ情報量が必要か確認する。
