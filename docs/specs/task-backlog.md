...
振り返り専用ページまたは詳細画面からの専用導線を検討する。現行の `PUT /api/wear-logs/{id}` は `items` を必須にする全体更新寄りの API であり、振り返り画面から既存 API を流用すると `items` / `source_outfit_id` の再構成ミスで着用記録本体を壊すリスクがある。backend 側は `PATCH /api/wear-logs/{id}/feedback` で `overall_rating` / 屋外・屋内の温度感 / `feedback_tags` / `feedback_memo` だけを更新できるため、frontend の振り返り専用導線ではこの API を使う。

frontend route はユーザー向け概念として `/wear-logs/{id}/reflection` を第一候補にする。UI 文言は「振り返り」、backend API / field 名は `feedback` として役割を分ける。MVP では detail 画面から `振り返りを書く` / `振り返りを編集する` 導線を出し、保存後は `/wear-logs/{id}` へ戻る。calendar / list からの直接導線は後続で検討する。

WearLogForm から服装フィードバック系 field を外すのは、reflection page と detail からの導線が実装できた後にする。そのタイミングで `has_feedback` / カレンダーアイコン / aria-label が「服装フィードバックあり」なのか「振り返りメモあり」なのかを再定義する。
