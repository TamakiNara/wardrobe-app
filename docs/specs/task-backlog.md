...
| UI-10 | 401 alert + login redirect のアプリ内通知化                    | `window.alert()` を使っていた `401` / セッション切れ / ログイン再要求通知を、current では `/login?message=session_expired` への `router.replace` と login page 側のアプリ内通知へ置き換え済み。component 側は共通 helper を使う。`return_to` / `next` によるログイン後復帰と logout failure alert は別タスクとして扱う。                                                                                 | `web/src` の alert 使用箇所, 手元確認メモ                                                                                                                                     | auth / error notification            | ui   | medium | current 方針実装済み / ログイン後復帰は保留    | yes              |
