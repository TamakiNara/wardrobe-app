...
| FEAT-07 | アイテムからコーディネートを検索する             | item detail または item list から、その item を含む outfit を検索・表示できるようにする。item 選択後の outfit 候補、件数表示、空状態、将来の wear log 登録導線も検討する                                                                                              | 手元タスク, `outfits/create-edit.md`, `items/status-management.md`                                         | items / outfits / wear logs          | feature | high   | outfit_items relation の検索 | needs design     |
...
- 着用履歴の振り返り関連は、表示条件 bug (`BUG-05`) と導線改善 (`UI-14`) に分けた
