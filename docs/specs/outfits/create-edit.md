...
- 現状の outfit 一覧では、item 構成の配色情報から生成する配色サムネイルを補助表示として出す
...
- 複製して新規作成を主導線とする
- 初期実装範囲では、invalid outfit の再利用は直接編集より複製して新規作成を主導線とし、手動復帰は条件を満たす場合のみ補助導線として扱う
...
- 複製を主導線、手動復帰を補助導線として配置
- `基本情報` セクションで memo / season / tpo をまとめて確認する
- invalid の現状の主導線は「詳細で状況確認 → 複製または復旧検討」とする
...
Outfit の新規作成 / 編集画面では、item 候補一覧に対して次の絞り込みを共通で使う。
...
絞り込みは候補一覧を見やすくするためのもので、選択済み item の保持とは分けて扱う。

- 絞り込み条件に合わなくなった item でも、すでに選択済みなら「選択中の順序」からは消さない
- カテゴリ未選択時は種類を disabled にする
- カテゴリ変更時は種類をクリアする
- item 一覧と同じカテゴリ・種類マスタを使い、カテゴリに応じて種類候補を切り替える
- 季節が未指定の初期表示では、ユーザー設定の「現在の季節」があれば item 候補の季節絞り込み初期値に反映する
- タグ機能は将来追加予定だが、現時点では未実装とし、方針メモは `docs/specs/tags.md` を参照する
- 候補カードでは、登録済み写真 URL がある場合だけ写真サムネイルを表示する。
- 写真がない item には placeholder や余計な thumbnail frame を出さない。
