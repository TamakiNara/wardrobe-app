...
- item 候補絞り込みの季節候補は、候補 item 実データから絞らず固定の季節候補を表示する
- ユーザー設定の「現在の季節」がある場合は、item 候補絞り込みの季節初期値に反映する
- 現在の季節に一致する候補 item が 0 件でも、初期値の季節表示自体は維持する

---

## item 候補一覧の絞り込み

Outfit の item 候補一覧では、次の条件で候補を絞れる。
...
- すでに outfit に追加済みの item は、現在の絞り込み条件に合わなくなっても選択済み表示から外さない
- 編集時も同じルールで、選択済み item は候補表示の可否とは分けて保持する
- カテゴリ未選択時は種類を disabled にする
- カテゴリ変更時は種類をクリアする
- 季節が未指定の初期表示では、ユーザー設定の「現在の季節」があれば item 候補の季節絞り込み初期値に反映する
- 季節で絞り込む場合は、その季節に加えて `オール` の item も候補に含める
