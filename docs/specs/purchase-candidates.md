...
- 色違い group の識別性向上として、main color にユーザー任意の色名ラベルを持たせるか検討する
  - 例: モカ、グレージュ、ブルーグレー、アイボリー寄りホワイト
  - 既存の color `label` はプリセット / custom 由来の表示名として扱い、ユーザー入力の任意ラベルとは責務を分ける
  - 初期候補は main color のみとし、sub color への追加は後続で再判断する
