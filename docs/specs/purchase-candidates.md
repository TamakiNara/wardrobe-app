...
- `sale_price` があり、`discount_ends_at` が未設定または現在時刻より未来の場合は sale 価格を主表示にし、通常価格を補助表示にする
- `discount_ends_at`（セール終了日）が過去の場合は、現在有効なセールとして扱わず、`セール中` badge / sale 価格の強調表示 / `セール終了日` 補助表示はしない
- `discount_ends_at` が未設定の `sale_price` は、既存データ互換のため current では sale 価格として扱う
- `discount_ends_at` が未来の場合は、セール終了日の日付部分を控えめに表示する
...
- API / BFF の表示用出力は `colors` / `seasons` / `tpos` の配列を使う
- item の TPO は `tpo_ids` ベースへ寄っており、表示時は `user_tpos.name` を解決し、必要に応じて legacy `tpos` を fallback とする
- `colors` / `seasons` は current では配列値として扱う
...
- `purchase_candidate_tpos.tpo` は TPO 名を保存する name snapshot であり、`tpo_ids` はまだ持たない
- purchase candidate form は settings の active `user_tpos` を選択肢に使うが、保存後は `user_tpos` の名称変更に追従しない

### 今フェーズの方針

- candidate 側の colors / seasons / tpos / materials は別テーブルで保存する
- item 側の TPO は `tpo_ids` ベース、candidate 側の TPO は name snapshot という差分を短期では許容する
...
- purchase candidate API は `tpos: string[]` を受け取り、`purchase_candidate_tpos.tpo` に name snapshot として保存する。
- `tpos` の選択肢は settings の active `user_tpos` から取得する。
- 保存後の purchase candidate は `user_tpos` の名称変更に追従しない。
- 複製 / 色違い追加では、purchase candidate の `tpos` name array をそのまま引き継ぐ。
- item 化 draft では `tpos` name array を渡し、item 作成側で active `user_tpos` と name match できる場合に `tpo_ids` 化できる。

短期方針:

- purchase candidate の TPO は今すぐ ID ベース化しない。
- current の name snapshot 運用と import/export 互換を維持する。

計画 / 要再判断:

- 将来 ID ベース化する場合は、`tpo_id` + legacy `tpo` fallback の dual model を検討する。
- API は `tpo_ids` preferred / `tpos` legacy の移行期間を検討する。
- item 化 draft は `tpo_ids` と `tpos` の両方を持つ案を検討する。
- inactive / renamed TPO の表示 fallback と legacy import の扱いを整理してから migration する。
...
| `tpos`             | `tpos`               | `tpos` / `tpo_ids`   | candidate は name snapshot。item 作成側で active `user_tpos` と name match できる場合に `tpo_ids` 化する |
...
- 購入検討一覧
  - 情報量を抑えるため、期限を出す場合はまず未来の `discount_ends_at` を `セール終了日` として補助表示する
  - `discount_ends_at` を過ぎた candidate は、現在有効な `セール中` としては表示せず、一覧では期限切れの `セール終了日` も表示しない
