...
- `sale_price` / `sale_ends_at` を candidate 側へ追加する
- candidate 一覧 / 詳細で sale 情報を補助表示する
- candidate 複製機能を追加し、詳細画面からの複製を主導線にする

### 要再判断

- candidate `memo` を item 初期値へ引き継ぎ続けるか
- sale 情報を item 作成画面でどこまで参考表示するか
- ホームで `considering` の sale 候補を要約表示するか
- `purchased` 後 candidate に許可する編集範囲
- candidate 複製時の画像ファイルの扱いを再利用にするか物理コピーにするか
...
- sale 情報 (`sale_price` / `sale_ends_at`) は未実装であり、current の `item-draft` / item create には含めない
...
- candidate の sale 情報保持
- candidate 一覧 / 詳細での sale 情報表示
- candidate 複製機能
