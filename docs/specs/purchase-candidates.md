...
- wear logs を含む全体再編は後続検討とし、MVP では購入検討を既存主要導線へ追加する
- 想定する主要ナビは以下
  - ホーム
  - items
  - outfits
...
添付メモや後続検討事項を current 実装へ混ぜないため、購入検討まわりの論点は次のように切り分ける。
...
- `purchased` 後 candidate では `memo` / `wanted_reason` / `priority` / `sale_price` / `sale_ends_at` / `purchase_url` / 画像のみ更新できる
...
- `converted_item_id` / `converted_at` のカラムは current schema に存在する
- current 実装では、`purchase_candidate_id` 付きの item 作成成功時に自動更新する
...
素材・混率は current で `docs/specs/items/material-composition.md` に沿って candidate 側まで実装済みとし、`materials[{ part_label, material_name, ratio }]` を detail / create / edit / item-draft で共通利用する。
...
| `memo` | `memo` | `memo` | current 実装では item 作成画面の初期値としてそのまま引き継ぐ。保存後は candidate と item で同期しない |
...
- sale 情報 (`sale_price` / `sale_ends_at`) は current でも candidate 専用であり、`item-draft` / item create には含めない
...
current 実装では、`purchase_candidate_id` を受けた item 作成時に、Laravel 側で次を同じ transaction 内にまとめる。
...
- `purchased` の candidate では `memo` / `wanted_reason` / `priority` / `sale_price` / `sale_ends_at` / `purchase_url` / 画像のみ編集可とし、その他の項目は編集不可とする
