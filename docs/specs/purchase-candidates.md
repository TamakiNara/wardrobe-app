...
- `sale_price` / `sale_ends_at` は candidate 専用の補助情報として保持する
...
- `purchased` 後 candidate では `memo` / `wanted_reason` / `priority` / `sale_price` / `sale_ends_at` / `purchase_url` を更新できる
...
### main color 任意色名ラベルの検討

色違い group の識別性向上として、main color にユーザー任意の色名ラベルを持たせることを検討する。
...
`purchase_candidate_colors` に `custom_label` nullable を追加する案を第一候補とする。

- `role = main` の行だけ入力対象とする
- `role = sub` では使用しない
- DB 制約で main のみに限定するより、初期は validation / form 側で制御する
...
- 通常複製では `custom_label` を引き継ぐ
- 色違い追加では、別色への変更を前提とするため `custom_label` は空にする
...
- candidate 側に `custom_label` を追加する
- purchase candidate form / list / detail / duplicate / color-variant に対応する
...
- `sale_ends_at` がある場合は、終了予定の日付部分を控えめに強調する
...
- 初回は通常複製と同じ初期値引き継ぎでよい
- 名前、カテゴリ、ブランド、価格、sale 情報、URL、memo、wanted_reason、materials、images、colors などは一旦引き継ぎ対象としてよい
- 保存前に色や画像を差し替えて使う前提とする

### 将来の改善候補

- 色違い追加時だけ、色や画像を引き継がず空で始める専用挙動を持たせるかは後続で再判断する
...
- candidate 側の代表画像切り替え UI と並び替え UI は未対応であり、後続対応とする
- item 側では保存後画像の並び替えと代表画像切り替えを編集画面で扱う
...
- candidate 画像追加 UI は file select / drag & drop / paste をサポートする
...
- sale 情報 (`sale_price` / `sale_ends_at`) は 現時点でも candidate 専用であり、`item-draft` / item create には含めない
...
- candidate 側の代表画像変更と並び替えは後続対応とする
...
candidate 側の代表画像切り替え UI と並び替え UI は後続対応とする。
...
- `sale_ends_at` は `datetime-local` ではなく、日付入力と時刻入力に分ける
- `sale_ends_at` の日付を選んだ時点で、内部値は `YYYY-MM-DDT00:00` とする
- `sale_ends_at` の時刻入力は、日付未選択時は無効にする
- 編集 / duplicate / color-variant draft では、既存の `sale_ends_at` 時刻を維持する
- `sale_ends_at` はリセットボタンで空に戻せるようにし、保存時は `null` 相当で送る
- `purchased` の candidate では `memo` / `wanted_reason` / `priority` / `sale_price` / `sale_ends_at` / `purchase_url` を編集可とし、その他の項目は編集不可とする
...
- ホームでの sale 候補の要約表示
- 月次服飾費集計の詳細
- candidate 側画像の代表画像切り替え / 並び替え UI
- main color 任意色名ラベルの詳細は `## 色違いグループ` を参照する
