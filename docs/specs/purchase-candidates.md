...
購入検討(管理)（内部識別子: `purchase_candidates`）機能の仕様を定義する。  
この資料では、購入検討(管理)の役割、`items` との責務分離、状態管理、item への昇格、画像、画面、API、集計前提を整理する。

---

## 概要

購入検討(管理)は、クローゼット系 Web アプリにおける **「まだ所持していないが、購入を検討している服」** を管理するための機能とする。
...
- 購入検討(管理)は **準主要機能** とする
- 購入検討(管理)は主要導線としてボトムナビに含める
- wear logs を含む全体再編は後続検討とし、MVP では購入検討(管理)を既存主要導線へ追加する
- 想定する主要ナビは以下
  - ホーム
  - items
  - outfits
...
- 購入検討(管理)は `items` と **別エンティティ** で管理する
- item 化は「変換」ではなく **item 新規作成** とする
- 購入検討(管理)と所持品は責務を混ぜない
...
- candidate 画像は購入検討(管理)側で upload / delete し、購入判断材料として扱う
