...
- ただし current 実装では、item 保存成功時の自動更新は未対応とする
...
- item 保存成功時に candidate を `purchased` に更新する
- `converted_item_id` / `converted_at` を保存する
...
current 実装では、item 保存成功時に candidate を `purchased` に更新しない。
