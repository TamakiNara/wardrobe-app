...
- 保存済み地域だが `forecast_area_code` がない場合
  - `この地域は予報取得に未対応です。地域設定で予報用コードを設定してください。`
- 今回だけの地域タブの場合
  - `予報取得は保存済み地域のみ対応です。`
...
- `memo` は自動入力しない
- 画面上に `予報をフォームに反映しました。保存はまだされていません。` を表示する
...
- `POST /api/weather-records/fetch-forecast`
...
```json
{
  ...
  "forecast": {
    ...
    "matched_forecast_date": "2026-05-01",
    "raw_telop": "晴れ時々くもり"
  }
}
```
...
- `予報の取得に失敗しました。時間をおいて再度お試しください。`
- `この地域の予報取得に必要な設定が不足しています。`
- `選択した日付の予報を取得できませんでした。`
