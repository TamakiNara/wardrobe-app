...
- external API failure / geocoding no result は current で Laravel structured log を残す
...
- log には `provider / source_type / user_id / location_id / weather_date / elapsed_ms` を中心に含め、raw provider response 全文は出さない
