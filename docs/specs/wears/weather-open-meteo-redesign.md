...
- Open-Meteo 移行に向けた土台として、weather location は latitude / longitude / 	imezone を保存できる
- DB 上は既存地域互換のため nullable のままにし、Open-Meteo 取得時に座標未設定を判定する方針を維持する
- 地域設定 UI では手入力 fallback の座標欄を追加し、日本国内向けの Asia/Tokyo を案内する

### planned

- forecast / historical API の正本入力は location_id から解決した latitude / longitude / 	imezone に寄せる
