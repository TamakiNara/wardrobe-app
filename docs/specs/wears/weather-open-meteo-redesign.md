...
- `POST /api/weather-records/observed` は、保存済み地域に `latitude` / `longitude` がある場合に Open-Meteo Historical API を使って実績相当データを取得する
- endpoint は `https://archive-api.open-meteo.com/v1/archive` を使い、`latitude` / `longitude` / `timezone` / `start_date` / `end_date` / `daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_sum,rain_sum,snowfall_sum,precipitation_hours` を指定する
- `timezone` が未設定の地域は `Asia/Tokyo` を fallback として使う
- daily `weather_code` を代表天気として app `weather_code` へ変換する
- `source_type = historical_api` / `source_name = open_meteo_historical`
- `precipitation` / `rain_sum` / `snowfall_sum` / `precipitation_hours` は response と画面の参考表示に含めるが、今回は DB 保存しない
- JMA latest CSV PoC は本線採用せず、legacy PoC の知見として docs のみに残す

### planned

- forecast / observed のどちらも `latitude` / `longitude` / `timezone` を正本として揃える
- `precipitation` 系の保存が必要になった場合は、`weather_records` への保存カラム追加を別途検討する
- 予報 snapshot / 実績 snapshot の分離や、予報値と実績値の差分保持は将来検討する
...
- Open-Meteo 移行後の天気登録画面では、`予報を取得` と `実績を取得` を 1 セクションに残したまま、対象日付に応じて主ボタンだけを切り替える案 B を採用候補とする
- 片方だけを常に表示する案 C は、今日の扱いが難しく、手動で予報と実績を切り替えたいケースも残るため採用しない
- 常に両方同格で並べる案 A は、実装は軽いが主導線が分かりづらいため優先しない

### 未来日

- `予報を取得` を主ボタンとして有効にする
- `実績を取得` は disabled にする
- 補足文候補:
...
- `実績を取得` を主ボタンとして有効にする
- `予報を取得` は disabled または補助扱いへ寄せる
- 補足文候補:
  - `過去日は実績データの取得を推奨します。`

### API と取得可能範囲の整理

- Open-Meteo forecast は、MVP では `未来日` と `今日` 向けの取得導線として扱う
- Open-Meteo historical は、MVP では `過去日` 向けの取得導線として扱う
- `今日` の historical は取得できる場合があっても未確定値を含む可能性があるため、UI 上で注意を出す
- 取得範囲外や upstream 側の制約で取得できない場合でも、手入力で続行できることを前提にする
