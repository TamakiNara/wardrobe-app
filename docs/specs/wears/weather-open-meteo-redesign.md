...
- `user_weather_locations` の current / planned / legacy の整理は [weather locations](../settings/weather-locations.md) を正本とする
...
- `weather_records` の保存方針や source metadata の current 意味は [weather-records.md](./weather-records.md) を正本とする
...
- `/dev/weather-preview` では Open-Meteo WMO weather code から app `weather_code` への変換を目視確認できる。
- JMA 天気文の変換一覧は legacy / fallback 確認用として残している。
...
- 天気登録画面では、対象日付に応じて forecast / historical のどちらを主導線にするかを UI 上で出し分けている。
...
- 今日は forecast を主ボタンにしつつ、historical も有効だが未確定注意文付きで扱う。
## 2026-05-03 weather record persistence redesign note

### current

- Open-Meteo forecast / historical はどちらもフォーム反映のみで、自動保存しない。
- 保存時には current のフォーム値だけが `weather_records` へ入る。
- `source_name = open_meteo_jma_forecast` / `open_meteo_historical` は、「最後にフォームへ反映した取得元」を示す。
- そのため、record 内で forecast 由来の値と historical 由来の値が混在する可能性がある。

### options comparison

#### 案A: 現行維持

- 最終保存値だけを `weather_records` に持つ。
- source は最後に反映した取得元として割り切る。

向いている点:

- 現在の開発規模に対して最も軽い。
- Open-Meteo 移行初期の実装を進めやすい。

弱い点:

- 予報値と実績値の差分を後から追えない。
- precipitation 系や raw 値を分析に使いにくい。

#### 案B: snapshot JSON を持つ
...
- forecast / historical / 最終保存値の 3 層を分けやすい。
- Open-Meteo の `raw_weather_code`、precipitation 系、source metadata をまとめやすい。
- import / export も record 単位で扱いやすい。

弱い点:

- JSON schema version と validation を持つ必要がある。

#### 案C: source 別テーブル

- `weather_record_sources` で履歴を持つ。
...
- MVP には重い。
- UI / cleanup / import-export が複雑になる。
...
- Open-Meteo 移行の短期では案Aを維持する。
- 中期の実装候補として案Bを第一候補にする。
- 案Cは履歴監査や分析要件が明確になってから検討する。
- 案Dは採用しない。

### precipitation / raw 値の扱い

- current では `precipitation` / `rain_sum` / `snowfall_sum` / `precipitation_hours` は参考表示のみとする。
- 将来保存するなら、最終保存値の直カラムより snapshot に含める案を優先する。
- `raw_weather_code` / `raw_weather_text` も snapshot または `source_payload` に寄せる案が自然である。
## 2026-05-03 calendar weather status note

### current

- Open-Meteo forecast / historical を使う current 設計でも、`weather_records` は最終保存値を持つ前提を維持する。
- カレンダー上では Open-Meteo の取得元そのものよりも、まず `weather_code` に対応する天気アイコンを主表示とする。
- source 状態は `forecast` / `observed` / `manual` を内部 status として持ち、主アイコンではなく色や補助表示で表す方針にする。
- `forecast` は sky / blue 系、`observed` は emerald / green 系、`manual` は observed 寄せまたは slate 寄りを第一候補にする。
- `CloudAlert` や `CloudCheck` などの状態アイコンを主表示にする案は採用しない。実際の天気を視覚的に把握しにくくなるため。
- `is_user_edited` は現時点では追加しない。snapshot を持たない current 設計では厳密判定が難しいため。

### planned

- カレンダーセルでは `天気アイコン -> 着用履歴 marker -> 振り返り CircleCheck` を第一候補にする。
- 日付詳細モーダルでは `予報` / `実績` / `手入力` の source status バッジを追加し、カレンダーより詳しく表示する。
- 将来 snapshot を持つ場合に、source ごとの差分表示や `is_user_edited` を再検討する。

### pending / 要再判断

- カレンダーセルの並び順を `着用履歴 -> 天気 -> 振り返り` と比較したうえで最終判断するか。
- `manual` を `observed` と同一トーンで見せるか、少し別のトーンにするか。
- source の言語化をカレンダーセルにどこまで出すか。
