...
- `weather_records` は current では最終保存値を保持する
- 予報取得後に保存した場合は `source_type = forecast_api` / `source_name = open_meteo_jma_forecast` を保存する
- 実績取得後に保存した場合は `source_type = historical_api` / `source_name = open_meteo_historical` を保存する
- source は「最後にフォームへ反映した取得元」を表す扱いであり、予報値と実績値の両方を同時保持する設計ではない

### planned

- forecast snapshot / observed snapshot の分離は将来検討する
- 予報値と実績値の差分保持や、どちらで上書きしたかの履歴保持は要再判断として残す
- precipitation 系の保存要否も、snapshot 分離の要否と合わせて再検討する
