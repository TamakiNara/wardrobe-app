...
- `overall_rating` / 温度感 / `feedback_tags` / `feedback_memo` のいずれかがある wear log は、その marker 自体を `CircleCheck` 系へ置き換えて「振り返りあり」を示す
- `planned` は白抜き系、`worn` は塗り系のまま維持し、`has_feedback=true` のときだけ同じ青系トーンの `CircleCheck` 版へ切り替える
- `CircleCheck` は「評価が良かった」ではなく「振り返り記録済み」を表し、セル右下などに独立した補助アイコンは置かない
