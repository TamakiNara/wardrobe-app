...
- wear logs では、配色サムネイルはまだ表示していない
- current の outfit 一覧サムネイル仕様を、そのまま wear logs へ流用した状態にはしていない

#### planned

- wear logs に配色サムネイルを導入する場合、正本は `source_outfit_id` ではなく `wear_log_items` を第一候補とする
- 理由:
  - current outfit の変更に引きずられにくい
  - `invalid outfit` や後編集の影響を受けにくい
  - snapshot なし前提でも、wear log 側 item 構成から描画できる
- 初期の適用先は、wear logs 一覧の補助表示候補として整理する
