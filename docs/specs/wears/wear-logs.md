...
現在の wear log 登録 / 編集フォームは、日付・状態・コーディネート・item・着用メモの登録と、服装フィードバック / 振り返り入力を同じ画面で扱っている。今後は、登録 / 編集画面を「いつ・何を着たか」の入力に寄せ、服装の振り返りは専用ページまたは専用導線へ分離する方針を検討する。
...
`memo` は登録時の補足として残す候補とし、`feedback_memo`、総合評価、屋外 / 屋内の温度感、feedback tag は着用後の振り返り側へ寄せる候補とする。ただし、既存の送信内容 / API は現時点では変更せず、UI 分離時に保存単位と表示導線を再判断する。
