...
- 日詳細モーダルに出す場合も、描画正本は `source_outfit_id` ではなく `wear_log_items` を維持する
- 日詳細モーダルでは、一覧カードと同じ helper を土台にしつつも、サイズや余白を抑えた簡略版 component に分ける案を第一候補とする
- 初期展開先は日詳細モーダルよりも個別詳細ではなく、まず日詳細モーダルを優先候補とする
- wear log 個別詳細にも出すか
- 色参照を current item 色のまま続けるか、snapshot 導入時にどう切り分けるか

#### 要再判断

- 日詳細モーダルで、一覧と同サイズのサムネイルにするか、より軽い簡略版にするか
