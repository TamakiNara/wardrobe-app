...
- item 単体では `onepiece_allinone`（shape は `onepiece` / `allinone`）も non-lower-body 系の共通四角文法で扱い、outfit 側は `bottoms` がない場合に限って最小限の `onepiece_allinone` 合成を持つ
...
- outfit サムネイルでは `legwear` を `others` へ入れず、lower-body 合成だけへ参加させる前提を固定し、`onepiece_allinone` 後続整理でもこの前提を使う
...
- outfit サムネイルの current では、`bottoms` がない場合に限り `onepiece_allinone` を全高レイヤー候補として扱う
...
- `bottoms` 併用時は `onepiece_allinone` レイヤーへ寄せず、current の通常レイアウトを維持する
- wear log は current では `wear_log_items` 正本の既存レイアウトを維持する

#### planned

- `onepiece_allinone` を含む outfit の重なり順ルールを wear log にも広げる
...
- `onepiece_allinone` + bottoms を current で許容ケースとして描くか、想定外組み合わせとして別扱いにするか
