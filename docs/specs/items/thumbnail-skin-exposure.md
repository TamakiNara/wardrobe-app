...
- current では wear log 専用の `onepiece_allinone` mode や representative 選定ルールはまだ持たず、既存の簡略レイアウトを維持する
- outfit thumbnail の current で確定している `legwear = lower-body 専用`、`tops` と `onepiece_allinone` の上下は `sort_order` 正本、という責務分離だけを wear log 後続整理の前提にする
