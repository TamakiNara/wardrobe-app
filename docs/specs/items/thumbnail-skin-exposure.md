...
- outfit サムネイルは既存レイアウトを持つが、lower-body はまだ未適用
- wear log サムネイルは `wear_log_items` を正本として扱う

### planned

- outfit サムネイルで lower-body preview を合成表示する
- outfit サムネイルでは既存ボトムス表示領域内で lower-body を再計算して描画する
- representative bottoms / representative legwear を選定して合成する
...
4. 該当なしの場合は representative legwear なしとする
...
- 旧データ互換として、描画側では欠落 / 無効値を legwear 効果なしとして扱う
...
- representative legwear は有効な `spec.legwear.coverage_type` を持つ item を優先し、全候補が無効な場合は legwear なしとして扱う
...
- 将来対応時は、bottoms 正本とは別ルールで裾丈・脚見え・legwear との重なりを再設計する
