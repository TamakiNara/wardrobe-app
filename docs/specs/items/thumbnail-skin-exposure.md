...
- item 単体では `dress` (`onepiece` / `allinone`) も non-lower-body 系の共通四角文法で扱い、outfit 側は `bottoms` がない場合に限って最小限の dress 合成を持つ
...
- wear log へ one-piece / dress の重なり順ルールを広げる
...
- outfit サムネイルの current では、`bottoms` がない場合に限り `dress` を全高レイヤー候補として扱う
- tops と `dress` が同時にある場合は、`sort_order` の大きい item を上側レイヤーとして描く
- `onepiece` は固定の裾位置を持つ簡略ルールで lower-body preview を下側に出し、`allinone` は full 扱いで lower-body を見せない
- `bottoms` 併用時は dress 専用レイヤーへ寄せず、current の通常レイアウトを維持する
- wear log は current では `wear_log_items` 正本の既存レイアウトを維持する

#### planned

- one-piece / dress を含む outfit の重なり順ルールを wear log にも広げる
...
- wear log では one-piece / dress を `others` のままにせず、tops との上下関係を `sort_order` 正本で決める別ルールを後続で整理する
