...
- item 単体では `dress` (`onepiece` / `allinone`) も non-lower-body 系の共通四角文法で扱い、outfit / wear log 側の合成表現はまだ持ち込まない
- outfit サムネイルでは、従来 `legwear` を色帯レイアウトの `others` へも流していたが、current では lower-body 専用責務へ寄せている

### planned

- wear log サムネイルでの合成表現は後続フェーズで整理する
- one-piece / dress を representative bottoms 判定対象へ含める場合の別ルールを整理する
- tights / stockings のサブカラー表現を固定する
- one-piece / dress を含む outfit は、tops との上下関係を category 固定優先ではなく `sort_order` を正本にして決め、wear log へも同じ考え方を広げられる形で整理する
- outfit サムネイルでは `legwear` を `others` へ入れず、lower-body 合成だけへ参加させる前提を固定し、one-piece / dress 後続整理でもこの前提を使う

### 要再判断

- 一覧の極小サイズで微差が見えにくい場合の簡略化
- tights / stockings のサブカラー表現
- one-piece / dress 将来対応時の別ルール
- `bottom_length_type` から描画割合への変換テーブルの最終値
- `wear_log_items` 内でボトムス / レッグウェア候補が複数ある場合の優先順位
- 透け感や厚みを将来どこまで表現するか
- one-piece / dress と bottoms の併用を current でどう扱うか、およびサムネイル上でどこまで想定外組み合わせを簡略化するか
...
- `tops` / `outer` / `dress` / `inner` / `shoes` / `accessories` は shape ごとの個別 SVG を使わず、カテゴリ単位の共通表現で描く
...
### one-piece / dress 系の扱い整理

#### current

- item 単体では、`dress` (`onepiece` / `allinone`) も non-lower-body 系カテゴリとして角丸四角ベースの共通 SVG を使う
- outfit / wear log の current サムネイルでは、`dress` は `others` グループとして扱われ、専用の全高レイヤーや lower-body 合成にはまだ参加しない
- そのため current では、one-piece / dress を含む outfit でも tops / bottoms / others の既存レイアウトを崩さず、wear log も `wear_log_items` 正本の同系統レイアウトを維持する

#### planned

- one-piece / dress を含む outfit は、item 並び順 (`sort_order`) を正本にして上下関係を決める
- one-piece / dress は「全高を持つ衣服レイヤー」の候補として扱い、その上に来る tops は上側レイヤーとして重ね、逆順なら tops の下に one-piece / dress を重ねる
- legwear は one-piece / dress がある場合でも下側レイヤーとして重ね、current lower-body preview の representative legwear 選定と描画ルールを流用する方向で整理する
- outer は current outfit の後段重ね責務を維持し、one-piece / dress があることだけで自動的に消したり最前面固定したりしない
- wear log は今回は実装対象外だが、将来は outfit と同じ「並び順正本」の重なり順ルールを `wear_log_items` にも適用できるようにする

#### 要再判断

- one-piece / dress + bottoms を current で許容ケースとして描くか、想定外組み合わせとして別扱いにするか
- `allinone` と bottoms のような不自然な組み合わせをサムネイルでどう簡略化するか
- one-piece / dress と tops の重なりを極小サイズでも読める最小限の図形ルールへどこまで落とすか
...
- outfit では one-piece / dress を `others` のままにせず、tops との上下関係を `sort_order` 正本で決める別ルールを後続で整理する
- legwear は下側レイヤーとして流用し、wear log へも同じ考え方を広げる前提で設計を詰める
- bottoms 併用は想定ケースと想定外ケースの境界を先に整理し、曖昧なまま representative bottoms へ含めない
