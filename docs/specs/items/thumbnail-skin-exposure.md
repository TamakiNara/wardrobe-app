...
- 透け感や厚みを将来どこまで表現するか
- `onepiece_allinone` と bottoms の併用を 現時点でどう扱うか、およびサムネイル上でどこまで想定外組み合わせを簡略化するか
...
- `spec.legwear.coverage_type`
- Phase 1 の 現状の UI では `legwear` category の item に対して扱う
...
- tops spec は 現時点では保存・表示に利用するが、item SVG 描画では shape 差分の判断に使わない
...
現時点では `skinTonePreset` を使い、item 単体の差分だけを表現する。
...
- lower-body preview の描画ルールは item 側の 現在のルールを再利用する
...
- 現時点では wear log 専用の `onepiece_allinone` mode を `onepiece + bottoms` に限って持ち、`allinone + bottoms` は `standard` mode を維持する
- 現時点では representative bottoms / legwear を `wear_log_items` から選び、representative bottoms がある場合のみ lower-body preview を表示する
- 現時点では `legwear` を `others` へ流さず、lower-body preview 専用責務として扱う
- 現時点では skin tone は wear log API payload に重複保存せず、settings の `skinTonePreset` を web 側で取得して lower-body preview と `onepiece_allinone` lower-body preview の描画へ渡す
- 現状の wear log 専用 mode では、`tops` と `onepiece_allinone` の上下は `sort_order` 正本で解決する
- outfit thumbnail の 現時点で確定している `tops` と `onepiece_allinone` の上下は `sort_order` 正本、という責務分離だけを wear log 後続整理の前提にする
...
- outfit サムネイルの 現時点では、`bottoms` がない `onepiece_allinone` と `onepiece + bottoms` を `onepiece_allinone` 専用 mode の対象にし、`allinone + bottoms` は通常レイアウトへ残す
...
- `onepiece + bottoms` は組み合わせ自体を許容し、現状の outfit サムネイルでは `onepiece` を全高の主レイヤー、`bottoms` を裾から少し見える lower-body 補助レイヤーとして簡略表示する
- 現状の compact は小さい thumbnail variant 向けの補助区分に留め、`onepiece main` / `bottoms` / tops 全体 / `others` の構造比率は一覧と詳細で変えない
- `allinone + bottoms` は 現時点では専用簡略化へ寄せず、通常レイアウトを維持する
- `onepiece + bottoms + legwear` では `legwear` は引き続き lower-body preview 専用で扱い、`others` へは戻さない
- wear log は 現時点では `wear_log_items` 正本の既存レイアウトを維持する
...
- legwear は `onepiece_allinone` がある場合でも下側レイヤーとして重ね、現在の lower-body preview の representative legwear 選定と描画ルールを流用する方向で整理する
- outer は現在の outfit の後段重ね責務を維持し、`onepiece_allinone` があることだけで自動的に消したり最前面固定したりしない
- wear log は今回は実装対象外だが、将来は outfit と同じ「並び順正本」の重なり順ルールを `wear_log_items` にも適用できるようにする
- wear log では outfit 現状の lower-body preview 責務分離や `onepiece_allinone` の前後判定方針は流用しつつ、mode 判定・representative 選定・ViewModel 組み立ては `wear_log_items` 前提で別途整理する
...
- `allinone + bottoms` を wear log では現状維持に留めるか、将来別 mode へ切り出すか
...
- outfit サムネイルは現在の outfit item 構成を正本とする
...
- tights / stockings のサブカラー表現は 現時点では固定しない
