...
describe("InvalidOutfitsList", () => {
  ...
  it("URL クエリの初期値を反映し、条件クリアで URL も戻す", async () => {
    ...
    const { default: InvalidOutfitsList } =
      await import("./invalid-outfits-list");
    ...
    expect(replaceMock).toHaveBeenCalledWith("/outfits/invalid", {
      scroll: false,
    });
  });
  ...
  it("ページャ操作で page クエリを更新する", async () => {
    ...
    expect(replaceMock).toHaveBeenCalledWith("/outfits/invalid?page=3", {
      scroll: false,
    });
  });
  ...
  it("invalid 一覧では確認導線と複製導線を表示し、編集導線は出さない", async () => {
    ...
    expect(container.textContent).not.toContain(
      "詳細で利用不可の項目を確認し、必要に応じて複製または復旧を検討します。",
    );
  });
});
