...
const GENERIC_TORSO_PATHS: Record<string, string> = {
  ...
  cardigan: "M110 74 Q126 60 150 60 Q174 60 190 74 L202 264 Q202 270 194 270 H106 Q98 270 98 264 Z",
  camisole: "M118 88 Q128 78 150 78 Q172 78 182 88 L190 262 Q190 270 184 270 H116 Q110 270 110 262 Z",
  tanktop: "M114 82 Q128 68 150 68 Q172 68 186 82 L196 262 Q196 270 188 270 H112 Q104 270 104 262 Z",
};
...
function ShirtCollar({
  ...
  return (
    <>
      <path d="M121 58 L179 58 L210 98 L90 98 Z" fill={mainColor} opacity="0.9" />
      <path
...
function CardiganDetails({ softHighlight }: { softHighlight: string }) {
  return (
    <>
      ...
      <path fill={softHighlight} d="M118 72 Q134 88 138 118 V262 H110 Z" opacity="0.9" />
      <path fill={softHighlight} d="M182 72 Q166 88 162 118 V262 H190 Z" opacity="0.9" />
    </>
  );
}
...
function KnitDetails({ softHighlight }: { softHighlight: string }) {
  return (
    <>
      <path fill={softHighlight} d="M118 70 Q132 82 150 82 Q168 82 182 70 Q176 96 150 100 Q124 96 118 70 Z" opacity="0.95" />
...
function HemBand({ softHighlight }: { softHighlight: string }) {
  return <path fill={softHighlight} d="M104 248 H196 V262 H104 Z" opacity="0.55" />;
}
...
function CamisoleStraps({
  ...
  return (
    <>
      <path fill={mainColor} d="M128 52 Q132 46 138 46 Q142 46 144 52 L140 88 H130 Z" />
      <path fill={mainColor} d="M172 52 Q168 46 162 46 Q158 46 156 52 L160 88 H170 Z" />
      <path fill={softHighlight} d="M126 88 Q136 100 150 100 Q164 100 174 88 Q168 118 150 122 Q132 118 126 88 Z" opacity="0.95" />
    </>
  );
}
...
function TanktopBinding({
  ...
  return (
    <>
      <path fill={softHighlight} d="M118 74 Q132 88 150 88 Q168 88 182 74 Q176 98 150 102 Q124 98 118 74 Z" opacity="0.95" />
      <path fill={mainColor} d="M114 82 Q124 92 132 114 L122 120 Q116 102 104 92 Z" opacity="0.25" />
      <path fill={mainColor} d="M186 82 Q176 92 168 114 L178 120 Q184 102 196 92 Z" opacity="0.25" />
    </>
  );
}
...
function ButtonColumn({
  ...
  return (
    <>
      <circle cx="150" cy="116" r="3.5" fill={mainColor} opacity="0.92" stroke={detailLineColor} strokeWidth="1.2" />
      <circle cx="150" cy="146" r="3.5" fill={mainColor} opacity="0.92" stroke={detailLineColor} strokeWidth="1.2" />
      <circle cx="150" cy="176" r="3.5" fill={mainColor} opacity="0.92" stroke={detailLineColor} strokeWidth="1.2" />
      <circle cx="150" cy="206" r="3.5" fill={mainColor} opacity="0.92" stroke={detailLineColor} strokeWidth="1.2" />
      <circle cx="150" cy="236" r="3.5" fill={mainColor} opacity="0.92" stroke={detailLineColor} strokeWidth="1.2" />
    </>
  );
}
...
export default function GenericTopsPreviewSvg({
  ...
  const bodyPath = GENERIC_TORSO_PATHS[visualShape] ?? GENERIC_TORSO_PATHS.blouse;
  const showSleeves = visualShape === "shirt" || visualShape === "blouse" || visualShape === "knit" || visualShape === "cardigan";
  ...
  return (
    ...
    >
      <defs>
        <filter id="sub-color-chip-shadow" x="-50%" y="-50%" width="200%" height="200%">
          <feDropShadow dx="0" dy="1.5" stdDeviation="1.5" floodColor="rgba(15,23,42,0.28)" />
        </filter>
      </defs>
      ...
      <g transform={`translate(${translateX} ${translateY}) scale(${scale})`}>
        ...
        {/* 形状ごとの首元や前立て */}
        {showShirtCollar && (
          <ShirtCollar mainColor={mainColor} detailLineColor={detailLineColor} />
        )}
        {shape === "cardigan" && <CardiganDetails softHighlight={softHighlight} />}
        {shape === "knit" && <KnitDetails softHighlight={softHighlight} />}
        {showSoftNeck && (
          <SoftBlouseNeck softHighlight={softHighlight} softNeckPath={softNeckPath} />
        )}
        {showStraps && (
        ...
        {showButtons && (
          <ButtonColumn mainColor={mainColor} detailLineColor={detailLineColor} />
        )}
