...
async function openCandidateList(container: HTMLElement, testId: string) {
...
async function openItemCandidateList(container: HTMLElement) {
  await openCandidateList(container, "wear-log-item-list-toggle");
}

async function openOutfitCandidateList(container: HTMLElement) {
  await openCandidateList(container, "wear-log-outfit-list-toggle");
}
...
describe("WearLogForm", () => {
  ...
  it("初期表示では item / outfit 候補一覧を閉じ、候補取得を遅延する", async () => {
    ...
    expect(
      container.querySelector('[data-testid="wear-log-item-search"]'),
    ).toBeNull();
    expect(
      container.querySelector('[data-testid="wear-log-outfit-search"]'),
    ).toBeNull();
  ...
  it("通常導線の新規登録で item のみを送信できる", async () => {
    ...
    expect(container.textContent).not.toContain("振り返り");
    expect(container.textContent).not.toContain("よかったこと");
    expect(container.textContent).not.toContain("気になったこと");
  ...
  it("item 1件選択時に関連コーディネート候補を表示し、候補選択で outfit ベースの payload に切り替える", async () => {
    ...
    expect(fetchAllPaginatedCandidatesMock).toHaveBeenLastCalledWith(
      "/api/outfits?item_id=1",
      "outfits",
    );
    expect(container.textContent).toContain(
      "このアイテムを含むコーディネート候補",
    );
    expect(container.textContent).toContain("白T休日コーデ");
    expect(container.textContent).toContain("季節: 春");
    expect(container.textContent).toContain("TPO: 休日");
  ...
  it("item 1件選択時の関連コーディネート候補取得に失敗した場合はエラーを表示する", async () => {
    fetchAllPaginatedCandidatesMock
      .mockResolvedValueOnce({
        status: 200,
        entries: [
  ...
  it("複数 item 選択時は関連コーディネート候補を表示しない", async () => {
    fetchAllPaginatedCandidatesMock
      .mockResolvedValueOnce({
        status: 200,
        entries: [
  ...
  it("候補外データを含む既存レコードでも編集画面が壊れない", async () => {
    ...
    expect(
      container.querySelector('[data-testid="overall-rating"]'),
    ).toBeNull();
    expect(
      container.querySelector('[data-testid="outdoor-temperature-feel"]'),
    ).toBeNull();
    expect(
      container.querySelector('[data-testid="indoor-temperature-feel"]'),
    ).toBeNull();
  });
  ...
  it("WearLogForm では振り返り入力を表示せず、編集時は既存 feedback を維持して送信する", async () => {
    vi.stubGlobal(
      "fetch",
      vi
        .fn()
        .mockResolvedValueOnce({
          ...
          json: async () => ({
            wearLog: {
              ...
              overall_rating: "good",
              feedback_tags: ["comfortable_all_day", "color_mismatch"],
    ...
    expect(container.querySelector('[data-error-key="memo"]')).not.toBeNull();
