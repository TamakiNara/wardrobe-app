...
function buildCalendarCells(month: string, weekStart: "monday" | "sunday") {
  ...
  const leadingBlankCount = weekStart === "monday"
    ? (firstDay.getDay() + 6) % 7
    : firstDay.getDay();
  const cells: Array<{
    key: string;
...
function buildQueryWithMonth(searchParams: URLSearchParams, nextMonth: string): string {
...
export default function WearLogCalendar({
  ...
  const [dayDetails, setDayDetails] = useState<WearLogByDateResponse | null>(null);
  ...
  async function openDayDetails(date: string) {
    ...
    try {
      ...
      if (error instanceof ApiClientError) {
        ...
        setDetailsError(error.data?.message ?? "日別詳細の取得に失敗しました。");
        return;
      }
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-3 self-start md:self-auto">
            <button
              type="button"
              onClick={() => router.replace(`${pathname}${buildQueryWithMonth(searchParams, shiftMonth(month, -1))}`, {
                scroll: false,
              })}
                      ...
                      {summary.dots.map((dot, index) => (
                        <span
                          ...
                          className={`h-2.5 w-2.5 rounded-full ${
                            dot.status === "worn" ? "bg-blue-600" : "bg-blue-300"
                          }`}
                        />
                      ))}
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-gray-950/35 p-4 md:p-6">
          <div className="max-h-[min(80vh,720px)] w-full max-w-2xl overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-gray-200 px-5 py-4">
              <div>
                <h3 className="text-lg font-semibold text-gray-900">日別詳細</h3>
              ...
              {loadingDetails ? (
                ...
                dayDetails.wearLogs.map((wearLog) => (
                  ...
                  >
                    ...
                    <p className="mt-2 text-xs text-gray-500">アイテム {wearLog.items_count} 件</p>
                ...
                <div className="space-y-3 rounded-2xl border border-dashed border-gray-300 bg-gray-50 px-4 py-5">
                  <p className="text-sm font-medium text-gray-900">この日の着用履歴はまだありません</p>
                  <p className="text-sm text-gray-600">選択した日付で新しく着用履歴を登録できます。</p>
                  <Link
                    href={buildCreateHref(selectedDate, dayDetails?.wearLogs.length ?? 0)}
