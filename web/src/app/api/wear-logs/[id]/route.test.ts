...
describe("/api/wear-logs/[id] route", () => {
  ...
  it("GET は Laravel の wear log 詳細 API へ転送する", async () => {
    ...
    const req = {
      headers: new Headers(),
      nextUrl: new URL("http://localhost:3000/api/wear-logs/1"),
    };
    ...
    expect(json.wearLog.id).toBe(1);
  });

  it("PUT は Laravel の wear log 更新 API へ転送する", async () => {
    global.fetch = vi
      .fn()
